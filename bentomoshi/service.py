    ...
    with torch.no_grad():
      ...
      async def recv_loop():
        while True:
          ...
          if not isinstance(data, bytes): print("received non-bytes message"); continue;
          if len(data) == 0: print("received empty message"); continue;
...
def main(): Moshi.serve_http(port=int(os.getenv("PORT", "3000")),
                             reload=os.getenv("RELOAD", "True").lower() in ['true', '1', 'y', 'yes'])
